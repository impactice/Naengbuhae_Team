    ...
    <div ref={wrapRef} className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-64 bg-white rounded-xl shadow-lg border border-gray-200 z-50 overflow-hidden">
          <div className="px-4 pt-3 pb-1 text-xs text-gray-500" style={{ fontWeight: 600 }}>
            냉장고 전환
          </div>
            ...
            {fridges.length === 0 ? (
              ...
              fridges.map((f) => (
                <button
                  ...
                  className={`w-full px-4 py-2.5 flex items-center gap-2 hover:bg-gray-50 text-left ${
                    f.id === selected?.id ? 'bg-gray-50' : ''
                  }`}
                >
                  <Refrigerator
                    className={`w-4 h-4 ${
                      f.id === selected?.id ? 'text-black' : 'text-gray-400'
                    }`}
                  />
